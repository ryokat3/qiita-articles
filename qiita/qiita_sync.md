...
Qiita 同期をする GitHub の repository を一つ用意します。できれば専用の repository を用意することをお勧めします。
...
Qiita の記事の同期をチェックする GitHub Actions を cron で定期的に実行されるのですが、利用者全員が同じ時間を設定すると、GitHub にも Qiita にも一斉に負担がかかるので、それを避けるため設定変更になります。

::: warn
cron の時間設定は変更する
:::

下記の例 `29 17 * * *` は 17:29 UTC なので日本時間だと毎日 02:29 JST に起動することになります。もちろん一週間に一度に設定でも、毎時間起動しても構いません。
...
現在の GitHub のメインとなるデフォルトのブランチ名が `main` なので、`qiita_sync.yml`の on.push.branches も `main` になっていますが `master` や他のブランチ名を使われている方は適宜変更をお願いします。
...
この２つのファイルを GitHub に push すると同期が始まります。最初にインストールした時のファイル名は記事の ID に .md を付けたものになります。ファイルの拡張子が `.md` である限りは、ファイル名の変更やディレクトリの移動は自由なので、`git pull` した後は分かりやすいファイル名に変更してください。

:::note info
ファイル名の変更、ディレクトリの移動は自由です
:::
...
`title` や `tags` は自由に変更できますが、`id` を変更したり、消去したりしてはいけません。また `id` は他の記事と共用はできないので、ファイルをコピーする時には `id` を消去してください。
...
記事を Qiita にアップロードした後に自動的に `id` が付加されます。その他にも GitHub 側でファイルの一部を書き換える場合があるので、`git pull` などで local の repository も最新に追従するようにしてください。
